      ...
      <div className="flex flex-col gap-3">
        ...
        <section className="overflow-hidden rounded-[8px] border border-[#dfe1e8] bg-white">
          ...
          <div className="p-3">
            ...
            <div className="overflow-x-auto rounded-[6px] border border-[#dfe1e8]"><table className="w-full min-w-[740px] border-collapse text-left"><thead className="bg-[#f5f7fa] text-[#8f9195]"><tr className="h-[38px]"><th className="px-4 text-[14px] font-normal leading-[22px]">{isChinese ? "开始时间" : "Start time"}</th><th className="px-4 text-[14px] font-normal leading-[22px]">{isChinese ? "结束时间" : "End time"}</th><th className="px-4 text-[14px] font-normal leading-[22px]">{isChinese ? "设备类型" : "Device type"}</th><th className="px-4 text-[14px] font-normal leading-[22px]">{isChinese ? "序列号" : "Serial number"}</th><th className="px-4 text-[14px] font-normal leading-[22px]">{isChinese ? "设备编号" : "Device ID"}</th><th className="px-4 text-[14px] font-normal leading-[22px]">{isChinese ? "状态" : "Status"}</th></tr></thead><tbody>{filteredRows.length ? filteredRows.map((row) => <tr key={row.id} className="h-[46px] border-t border-[#dfe1e8]"><td className="px-4 text-[14px] leading-[22px] text-[#141414]">{row.startedAt}</td><td className="px-4 text-[14px] leading-[22px] text-[#141414]">{row.endedAt}</td><td className="px-4 text-[14px] leading-[22px] text-[#141414]">KONE Connection 320</td><td className="px-4 text-[14px] leading-[22px] text-[#141414]">{row.serialNumber}</td><td className="px-4 text-[14px] leading-[22px] text-[#141414]">{row.deviceId}</td><td className="px-4 text-[14px] leading-[22px]"><SubtaskStatus status={row.status} language={language} /></td></tr>) : <tr><td colSpan={6} className="h-28 text-center text-[14px] text-[#8f9195]">{isChinese ? "暂无匹配的子任务" : "No matching subtasks"}</td></tr>}</tbody></table></div>
          </div>
